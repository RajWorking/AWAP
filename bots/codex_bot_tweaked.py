...
class BotPlayer:
    ...
    def _choose_order(self, controller: RobotController, bot_count: int):
        ...
        for o in orders:
            ...
            base_min_turns = 45 if has_cooking else 5
            ...
            # More aggressive estimation based on parallelization potential
            complex_order = num_ingredients >= 5 or cooking_count >= 2
            if complex_order:
                if num_ingredients >= 5 and cooking_count >= 2 and map_area >= 200:
                    # Large complex orders with multi-bot speedup
                    estimated_turns = (num_ingredients * 7 + cooking_count * 25) / speedup
                    safety = 1.05 if bot_count >= 2 else 1.15
                    # Accept if we have enough time OR profit/turn is good
                    profit_per_turn = (total_value - cost) / estimated_turns if estimated_turns > 0 else 0
                    if remaining_turns < estimated_turns * safety:
                        if remaining_turns < estimated_turns and profit_per_turn < 5:
                            continue
                else:
                    # Complex orders (5+ ingredients or 2+ cooking)
                    estimated_turns = num_ingredients * 7 + cooking_count * 30
                    # More lenient: accept if profitable enough or have enough buffer
                    profit_per_turn = (total_value - cost) / estimated_turns if estimated_turns > 0 else 0
                    if remaining_turns < estimated_turns * 1.15:
                        # Skip only if both unprofitable AND no time
                        if remaining_turns < estimated_turns and profit_per_turn < 3:
                            continue
            else:
                # Simple orders - be more aggressive
                estimated_turns = (num_ingredients * 7 + chop_count * 3 + cooking_count * 20) / speedup
                safety = 1.05 if bot_count >= 2 else 1.1  # Reduced from 1.1/1.2
                if remaining_turns < estimated_turns * safety:
                    # Allow tighter margins for simple orders
                    profit_per_turn = (total_value - cost) / estimated_turns if estimated_turns > 0 else 0
                    if profit_per_turn < 2:  # Only skip if really unprofitable
                        continue
            ...
            estimated_turns_base = (num_ingredients * 7 + chop_count * 3 + cooking_count * 20) / speedup
            profit_per_turn = profit / max(estimated_turns_base, 1)
            urgency = 1.0 - (remaining_turns / max(expires, 1))  # 0-1, higher = more urgent

            # Prioritize: feasibility, profit/turn, total profit, urgency
            # Less penalty for effort if profit is good
            effort_penalty = min(effort * 5, profit * 0.3)  # Cap effort penalty
            adjusted_reward = profit - effort_penalty

            score = (
                1 if feasible else 0,           # Must be affordable
                profit_per_turn,                 # Efficiency is key
                adjusted_reward,                 # Total adjusted profit
                -effort,                         # Prefer simpler orders when tied
                urgency                          # More urgent = better
            )
            scored.append((score, o, profit, estimated_turns_base))  # Store metrics for debug
        if not scored:
            return None
        scored.sort(key=lambda item: (-item[0][0], -item[0][1], -item[0][2], item[0][3], -item[0][4]))

        # Debug: print winner
        winner_score, winner_order, winner_profit, winner_est = scored[0]
        order_id = winner_order.get("order_id")
        print(f"  Order {order_id}: profit=${int(winner_profit)}, est_turns={int(winner_est)}, efficiency={winner_score[1]:.2f} profit/turn")
    ...
    def play_turn(self, controller: RobotController):
        ...
        # Only switch orders if we haven't made significant progress (no plate or no food on plate)
        # AND only if the better order is significantly better (50%+ efficiency improvement)
        if (plate_info is None or not plate_counter) and len(active_orders) > 1:
            ...
            if better is not None and better.get("order_id") != self.current_order_id:
                # Calculate efficiency difference before switching
                current_foods = self._order_foods(order)
                current_effort = sum(1 + (2 if f.can_chop else 0) + (3 if f.can_cook else 0) for f in current_foods)
                current_value = order.get("reward", 0) + order.get("penalty", 0)
                current_cost = sum(int(getattr(f, "buy_cost", 0)) for f in current_foods) + int(ShopCosts.PLATE.buy_cost)
                current_efficiency = (current_value - current_cost) / max(current_effort, 1)

                better_foods = self._order_foods(better)
                better_effort = sum(1 + (2 if f.can_chop else 0) + (3 if f.can_cook else 0) for f in better_foods)
                better_value = better.get("reward", 0) + better.get("penalty", 0)
                better_cost = sum(int(getattr(f, "buy_cost", 0)) for f in better_foods) + int(ShopCosts.PLATE.buy_cost)
                better_efficiency = (better_value - better_cost) / max(better_effort, 1)

                # Only switch if significantly better (50%+ improvement)
                if better_efficiency > current_efficiency * 1.5:
