...
class BotPlayer:
    ...
    def _choose_order(self, controller: RobotController, bot_count: int):
        ...
        # Multi-bot speedup factor (very aggressive for high throughput)
        speedup = 1.0 + 0.7 * (bot_count - 1) if bot_count > 1 else 1.0
        if speedup > 2.8:
            speedup = 2.8
        ...
        for o in orders:
            ...
            # OPTIMIZED TIME ESTIMATION for messy map
            base_turns = 18  # Account for messy map distances
            ingredient_turns = num_ingredients * 9  # More realistic gathering time
            chopping_turns = chop_count * 6  # Chopping overhead

            # Cooking time (more conservative)
            if cooking_count > 0:
                if bot_count > 1:
                    cooking_turns = cooking_count * 23  # Better multi-bot estimate
                else:
                    cooking_turns = cooking_count * 32  # Single bot
            else:
                cooking_turns = 0

            estimated_turns = (base_turns + ingredient_turns + chopping_turns + cooking_turns) / speedup

            # SAFER BUFFER to reduce failures
            game_turns_remaining = 500 - current_turn
            if game_turns_remaining < 150:
                buffer_multiplier = 1.30  # More conservative late game
            elif game_turns_remaining < 300:
                buffer_multiplier = 1.20  # Conservative mid game
            else:
                buffer_multiplier = 1.15  # Moderate early game
        ...
        # Calculate comprehensive score balancing profit and efficiency
        for c in candidates:
            # Balance absolute profit with efficiency
            base_score = c['profit'] * 0.7 + c['profit_per_turn'] * 35

            # Bonus for high-value orders (these are worth the effort)
            if c['profit'] > 250:
                base_score += 150
            elif c['profit'] > 180:
                base_score += 80
            ...
            # Small penalty for very complex orders (risk of failure)
            if c['num_ingredients'] > 3:
                base_score -= 25

            # Penalty for tight deadlines (risky)
            slack = c['remaining_turns'] - c['estimated_turns']
            if slack < c['estimated_turns'] * 0.3:
                base_score -= 40  # Too tight, risky

            c['score'] = base_score

        if game_turns_remaining < 120:
            # Late game: prioritize fast, profitable orders
            candidates.sort(key=lambda c: (-c['score'], c['estimated_turns']))
        elif game_turns_remaining < 280:
            # Mid game: balance score and avoid complexity
            candidates.sort(key=lambda c: (-c['score'], c['num_ingredients']))
        else:
            # Early game: maximize absolute profit with efficiency consideration
            candidates.sort(key=lambda c: (-c['profit'], -c['profit_per_turn'], c['estimated_turns']))
    ...
    def _ensure_pan(self, controller: RobotController, bot_id: int) -> bool:
        ...
        current_pan = getattr(tile, "item", None)

        # Pan with burnt food? Clear it first
        if isinstance(current_pan, Pan) and isinstance(current_pan.food, Food):
            if current_pan.food.cooked_stage >= 2:
                # Need to clear burnt food
                return False

        # Pan already present and empty/cooking correctly
        if isinstance(current_pan, Pan):
            return True
    ...
    def _take_cooked_food(self, controller: RobotController, bot_id: int, missing: Counter) -> bool:
        ...
        # PRIORITY: Remove burnt food immediately to free cooker
        if food.cooked_stage >= 2:
            return self._move_or_action(controller, bot_id, cx, cy, lambda: controller.take_from_pan(bot_id, cx, cy))

        # Take properly cooked food if needed
        if food.cooked_stage == 1 and self._food_needed(missing, food.food_name):
            return self._move_or_action(controller, bot_id, cx, cy, lambda: controller.take_from_pan(bot_id, cx, cy))
        ...
        def build_candidates(apply_roles: bool):
            ...
            for name, count in missing.items():
                ...
                if role == "cook" and ft.can_cook:
                    bonus += 5  # Cook should focus on cooking
                if role == "prep" and ft.can_chop:
                    bonus += 4  # Prep should focus on chopping
                if role == "plate" and (not ft.can_chop and not ft.can_cook):
                    bonus += 3
                if role == "runner":
                    bonus += 2  # Runners help with simple items

                # Prioritize expensive/complex ingredients slightly
                value_bonus = 0
                if ft.buy_cost > 40:
                    value_bonus = 1
                elif ft.buy_cost > 25:
                ...
                items.append((bonus + value_bonus, -steps, -ft.buy_cost, ft))
            return items
    ...
    def play_turn(self, controller: RobotController):
        ...
        if len(bot_ids) == 1:
            ...
            if not self.single_plan or self.single_plan_idx >= len(self.single_plan):
                if self._clear_hands(controller, bot_id):
                    return
            if not self.single_plan or self.single_plan_idx >= len(self.single_plan):
                # Use improved order selection for single bot
                remaining_orders = [o for o in active_orders if o.get("order_id") not in self.single_processed_orders]
                if not remaining_orders:
                    return
                # Choose best order using the improved selection method
        ...
        if order is None or self.assembly_counter is None:
            self.current_order_id = None  # Order no longer exists, clear it
            return
        ...
        # Check if order was just completed (plate submitted)
        if self.current_order_id is not None:
            plate_exists = self._get_plate_location(controller, bot_ids) is not None
            if not plate_exists and not missing:
                # Plate was submitted and order is complete
                self.completed_orders.add(self.current_order_id)
                self.current_order_id = None
