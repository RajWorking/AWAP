...
class BotPlayer:
    ...
    def _choose_order(self, controller: RobotController, bot_count: int):
        ...
        for o in orders:
            ...
            # AGGRESSIVE TIME ESTIMATION for high throughput
            base_turns = 15  # Reduced overhead with good coordination
            ingredient_turns = num_ingredients * 7  # Optimized with parallel execution
            chopping_turns = chop_count * 5  # Fast chopping with coordination

            # Cooking time with aggressive parallelization
            if cooking_count > 0:
                # With multi-bot, cooking can overlap significantly
                if bot_count > 1:
                    # Multiple bots can prep while cooking happens
                    cooking_turns = cooking_count * 20  # Overlapped cooking time
                else:
                    # Single bot sequential
            ...
            # AGGRESSIVE BUFFER for high throughput
            game_turns_remaining = 500 - current_turn
            if game_turns_remaining < 150:
                # Late game: still aggressive but ensure completion
                buffer_multiplier = 1.15
            elif game_turns_remaining < 350:
                # Mid game: very aggressive
                buffer_multiplier = 1.05
            else:
                # Early game: extremely aggressive - trust the speedup
        ...
        # AGGRESSIVE STRATEGY for maximum throughput
        game_turns_remaining = 500 - current_turn

        # Calculate aggressive score that heavily weighs both profit and efficiency
        for c in candidates:
            # Higher weight on absolute profit to chase high-value orders
            c['aggressive_score'] = c['profit'] * 0.6 + c['profit_per_turn'] * 50

        if game_turns_remaining < 150:
            # Late game: fast orders with good profit
            candidates.sort(key=lambda c: (c['estimated_turns'] * 0.5 - c['profit'] * 0.1, -c['profit']))
        else:
            # Early/mid game: maximize aggressive score
            candidates.sort(key=lambda c: -c['aggressive_score'])
