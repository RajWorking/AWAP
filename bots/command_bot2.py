...
class RecipePlanner:
    ...
    def build_commands_for_order(order: Dict) -> List[Command]:
        ...
        # Step 2: Process ingredients with cooking optimizations
        if cooking_foods:
            ...
            commands.append(PlaceOnCounterCommand())  # Place cooked food on counter

            # Add the cooked ingredient to plate
            commands.append(PickupPlateFromCounterCommand())
            commands.append(AddFoodToPlateCommand(first_cooking))
            commands.append(PlaceOnCounterCommand())
            ...
            # No cooking ingredients, just process non-cooking ones
            for food_type in non_cooking_foods:
                commands.append(BuyIngredientCommand(food_type))
